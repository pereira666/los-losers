...
const antilink = JSON.parse(fs.readFileSync('./database/json/antilinkhard.json'))
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const premium = ["558499034946@s.whatsapp.net","557588265283@s.whatsapp.net"] 
		...
		case 'bpfont':
			...
			break 
                ...
                case 'tempbot':
                    ...
                    client.updatePresence(from, Presence.composing) 
				uptime = process.uptime()
                    client.sendMessage(from, `O LOSER BOT ESTA ONLINE AS\n*${kyun(uptime)}*`, text, { quoted: mek})
                    break
				case 'delete':
				case 'del':
				case 'd':  
           ...
           case 'playmp3':
            ...
            case 'smule':
                ...
                sendImage(hasil, mek, '*GELAP BOS :V*')
                  break
               case 'tadas':
				 data = fs.readFileSync('./src/tadas.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*Loser Cantadas :V*')
                   break
                case 'wallpaper':
...
break
case 'tapa':
tujuh = fs.readFileSync('./assets/tapa.mp4');
client.sendMessage(from, tujuh, MessageType.video, {quoted: mek, mimetype: 'video/mp4', ptt:true})
break
case 'botada':
...
break
case 'pedro':
tujuh = fs.readFileSync('./img/pedro.webp');
client.sendMessage(from, tujuh, MessageType.figurinha, {quoted: mek, mimetype: 'figurinha/webp', ptt:true})
break
case 'xxxedit':
         ...
         case 'roleta':
           ...
           case 'casal':
					...
					if (!isGroup) return reply(mess.only.group)
						...
						teks = `*Hmmm.... Eu Shipo eles 2💘💘*\n\n1= @${teupai11.jid.split('@')[0]}\ne esse\n2= @${teupai21.jid.split('@')[0]}\ncom uma porcentagem de: ${shipted}De chance`
		    ...
		    case 'ship':
					...
					if (!isGroup) return reply(mess.only.group)
						const gay0 = split
						const gays1 = mentionedSplit
						const tcholada = gay0[Math.floor(Math.random() * gay0.length)]
						const tchola = gays1[Math.floor(Math.random() * gays1.length)]
						var ment = ["1%", `10%`, `20%`, `40%`, `50%`, `60%`, `80%`, `90%`, `100%`, `99999%`]
						const shhiped = ment[Math.floor(Math.random() * ment.length)]
						teks = `*Hmmm.... Eu Shipo eles 2💘*\n\n1= @${tcholada.jid.split('@')[0]}\ne esse\n2= @${tchola.jid.split('@')[0]}\ncom uma porcentagem de: ${shhiped}De chance`
						mentions(teks, split, true)
					   break
           case 'feio':
                    reply(mess.ficha)
					if (!isGroup) return reply(mess.only.group)
...
break
			   ...
			   case 'gay':		
					...
					break
              case 'pergunta':
                	if (args.length < 1) return reply('marque (@) e faça uma pergunta (lindo)!')
                    rate = body.slice(1)
                    const carlus =['Talvez sim','Talvez não','Prefiro não entrar nesse assunto!','Definitivamente não','Você pode acreditar nisso','¯\_(ツ)_/¯','Melhor não falar sobre isso agora','Não quero falar disso','Não necessariamente','Não','Sim','As vezes sim']
					const gun = carlus[Math.floor(Math.random() * carlus.length)]
                    client.sendMessage(from, 'Ola: *'+rate+'*\n\n*Resposta*: '+gun+' ', text, { quoted: mek })
					break
              case 'xingar':
				...
				break 
					...
					if (Number(args[0]) === 1) {
						...
						reply('O Loser ativou o anti-link ativado com sucesso neste grupo ✔️')
						...
						fs.writeFileSync('./src/antilinkhard.json', JSON.stringify(antilink))
                ...
                case '8bit':
                  ...
                  var bi2 = bi.split("|")[0];
                   ...
                   reply(mess.wait)
                   bu = await fetchJson(`https://api.xteam.xyz/photooxy/8bit?text=Xteam&text2=st4rz&APIKEY=APIKEYMU`, {method: 'get'})
...
// recursos anime, random @Loser domina vida
          ...
          reply('Espere..')
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					nneoo = body.slice(6)
					reply(mess.wait)
					nooe = await getBuffer(`https://api.xteam.xyz/textpro/neontext?text=Xteam&APIKEY=APIKEYMU{nneoo}`)
                ...
                case 'mascote':
				    reply(mess.aguarde)
					buffer = await getBuffer(`https://i.ibb.co/nsvy2Fp/IMG-20210407-WA1095.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Whatsapp Jr.'})
					break
                case 'tamu':
				    reply(mess.aguarde)
                ...
                case 'loli':
				    reply(mess.aguarde)
					buffer = await getBuffer(`https://i.ibb.co/qBJFxW2/Tijolao.jpg`)
					buffer = await getBuffer(`https://i.ibb.co/9r3WJQh/imge.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hmmm... Então Quer Ver Loli?'})
					break
                case 'buceta':
				    reply(mess.aguarde)
                ...
                case 'pe':
					...
					buffer = await getBuffer(`https://i.ibb.co/SvdhtxB/imageok.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Toma um pé ae'})
					break
                case 'gostosa':
				    reply(mess.aguarde)
					buffer = await getBuffer(`https://i.ibb.co/SPGc0gL/gosto.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'krl 😳'})
					break
				case 'hentai':
				    reply(mess.aguarde)
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.ibb.co/170qshS/donofds.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CREATED BY:* CARLOSX\n*CHAME O DONO PARA DÚVIDAS OU AFINS:* wa.me/+558499034946\n*INSTA DO DONO:* @carluus7\n\n\nUsem e se divirtam-se com o Loser Bot 🤖️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
